-- Keep suggestions small while allowing authenticated users to find more people on demand.
...
AS $$
  ...
  FROM (
    ...
    WHERE input.user_id IS NOT NULL
      ...
      AND (
        LOWER(profile.display_name) LIKE '%' || REPLACE(REPLACE(REPLACE(LOWER(input.query), '\', '\\'), '%', '\%'), '_', '\_') || '%'
        OR LOWER(profile.username) LIKE '%' || REPLACE(REPLACE(REPLACE(LOWER(input.query), '\', '\\'), '%', '\%'), '_', '\_') || '%'
      )
      AND NOT EXISTS (
