...
test('blocking preserves active and archived conversations through refresh and realtime updates', () => {
  ...
  try {
    ...
    store.updateConversation('active', { is_archived: true })
    store.updateConversation('active', { is_archived: true })
    expect(useChatsListStore.getState().conversations).toHaveLength(0)
    expect(useChatsListStore.getState().archivedConversations).toHaveLength(2)
    store.updateConversation('active', { is_archived: false })
    store.updateConversation('active', { is_archived: false })
    expect(useChatsListStore.getState().conversations).toHaveLength(1)
    expect(useChatsListStore.getState().archivedConversations).toHaveLength(1)
